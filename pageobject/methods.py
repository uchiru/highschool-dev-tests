...
def buy_all_inclusive(browser, card_1, mmyy_1, cvv_1, paypass_1):
    ...
    choose_all_inclusive = wdw(browser, 30).until(
    ...
    mmyy = wdw(browser, 30).until(EC.visibility_of_element_located(card_data_page_elements.card_date)).send_keys(
        card_data.mmyy_1)
    cvv = wdw(browser, 30).until(EC.visibility_of_element_located(card_data_page_elements.cvv_num)).send_keys(
        card_data.cvv_1)
    ...
    paid_click = wdw(browser, 40).until(
...
def new_subjects_year(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):
    ...
    paypass_click = wdw(browser, 30).until(
        EC.visibility_of_element_located(cloud_page_elements.paypass_button)).click()
    paid_click = wdw(browser, 40).until(EC.visibility_of_element_located(congrat_page_elements.congrat_button))
...
def new_subjects_halfyear(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):  # Ученик 5й класс
    ...
    full_version = wdw(browser, 30).until(
        EC.visibility_of_element_located(headpage_elements.full_version_button)).click()
    gradusnick_click = wdw(browser, 35).until(
...
def teach_sub_management(browser, choose_subjects):  # учитель-5й класс
    ...
    try:
        ...
        print("Pop up lose")
    header_click = wdw(browser, 30).until(
        EC.visibility_of_element_located(teachers_elements.header_head_page_button)).click()
    browser.implicitly_wait(15)


def teach_sub_management_7th(browser, choose_subjects):  # учитель-5й класс
    browser.get('https://57772.shot-uchi.ru/teachers/lk/main')
    subjects_list = wdw(browser, 30).until(
        EC.visibility_of_element_located(teachers_elements.subjects_list_button)).click()
    browser.implicitly_wait(15)
...
def regist_and_choose_classes(browser):  # учитель-5й класс
    ...
    literature_group_option = wdw(browser, 30).until(
    ...
    group_option_letter_button = wdw(browser, 30).until(
        EC.visibility_of_element_located(registraition_elements.class_option_letter_button)).click()
    browser.implicitly_wait(20)
    try:
        onboarding_1_button = wdw(browser, 30).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_2_button = wdw(browser, 30).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_3_button = wdw(browser, 30).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_4_button = wdw(browser, 30).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_5_button = wdw(browser, 30).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
    except:
        print("Pop up lose")
    have_biology = wdw(browser, 30).until(EC.visibility_of_element_located(teachers_elements.biology_at_menu))
    have_literature = wdw(browser, 30).until(EC.visibility_of_element_located(teachers_elements.literature_at_menu))
    subjects_list = wdw(browser, 30).until(
        EC.visibility_of_element_located(teachers_elements.subjects_list_button)).click()
    target = browser.find_element_by_css_selector(
        '[data-qa-marker="submit-editable-class"]').location_once_scrolled_into_view
    try:
        marathon_button = wdw(browser, 30).until(
