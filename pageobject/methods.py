...
def buy_all_inclusive(browser, card_1, mmyy_1, cvv_1, paypass_1):
    ...
    choose_all_inclusive = wdw(browser, 25).until(
    ...
    card = wdw(browser, 40).until(EC.visibility_of_element_located(card_data_page_elements.card_num)).send_keys(
        card_data.card_1)
    mmyy = wdw(browser, 25).until(EC.visibility_of_element_located(card_data_page_elements.card_date)).send_keys(
        card_data.mmyy_1)
    cvv = wdw(browser, 25).until(EC.visibility_of_element_located(card_data_page_elements.cvv_num)).send_keys(
        card_data.cvv_1)
    card_buy_click = wdw(browser, 25).until(
        EC.visibility_of_element_located(card_data_page_elements.card_buy_button)).click()
    browser.implicitly_wait(10)
    paypass = wdw(browser, 25).until(
        EC.visibility_of_element_located(cloud_page_elements.password_input)).send_keys(card_data.paypass_1)
    paypass_click = wdw(browser, 25).until(
        EC.visibility_of_element_located(cloud_page_elements.paypass_button)).click()
    paid_click = wdw(browser, 25).until(
...
def new_subjects_year(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):
    ...
    buy_click = wdw(browser, 25).until(EC.visibility_of_element_located(payments_page_elements.buy_button)).click()
...
def new_subjects_year_8th(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):
    browser.get('https://57772.shot-uchi.ru/profile/students')
    full_version = wdw(browser, 25).until(
        EC.visibility_of_element_located(headpage_elements.full_version_button)).click()
    if choose_checkbox == 'soc':
        society_click = wdw(browser, 30).until(
            EC.visibility_of_element_located(payments_page_elements.society_check_box)).click()
    elif choose_checkbox == 'phys':
        phys_click = wdw(browser, 30).until(
...
def old_subjects_year(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_check_box):  # ученик старые предметы
    ...
    if choose_check_box == 'math':
        ...
        plan_click = wdw(browser, 25).until(
...
def new_subjects_halfyear_8th(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):  # Ученик 7й класс
    browser.get('https://57772.shot-uchi.ru/profile/students')
    full_version = wdw(browser, 25).until(
        EC.visibility_of_element_located(headpage_elements.full_version_button)).click()
    gradusnick_click = wdw(browser, 25).until(
...
def new_subjects_month_8th(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):  # ученик 7йкласс
    browser.get('https://57772.shot-uchi.ru/profile/students')
    full_version = wdw(browser, 25).until(
        EC.visibility_of_element_located(headpage_elements.full_version_button)).click()
    gradusnick_click = wdw(browser, 25).until(
...
def teach_sub_management(browser, choose_subjects):  # учитель-5й класс
    ...
    if choose_subjects == 'bio':
        bio_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.bio_check)).click()
    # elif choose_subjects == 'geo':
    #     geo_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.geo_check)).click()
    # elif choose_subjects == 'hist':
    #     hist_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.hist_check)).click()
    elif choose_subjects == 'lit':
        lit_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.lit_check)).click()
    else:
        bio_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.bio_check)).click()
        lit_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.lit_check)).click()
        # geo_choose = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.geo_check)).click()
...
def regist_and_choose_classes(browser):  # учитель-5й класс
    ...
    teach_school_click = wdw(browser, 25).until(
    ...
    literature_group_option = wdw(browser, 25).until(
    ...
    group_option_letter_button = wdw(browser, 25).until(
        EC.visibility_of_element_located(registraition_elements.class_option_letter_button)).click()
    browser.implicitly_wait(20)
    try:
        onboarding_1_button = wdw(browser, 25).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_2_button = wdw(browser, 25).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_3_button = wdw(browser, 25).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_4_button = wdw(browser, 25).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
        onboarding_5_button = wdw(browser, 25).until(
            EC.visibility_of_element_located(teachers_elements.your_class_hear_pop_up)).click()
    except:
        print("Pop up lose")
    have_biology = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.biology_at_menu))
    have_literature = wdw(browser, 25).until(EC.visibility_of_element_located(teachers_elements.literature_at_menu))
    subjects_list = wdw(browser, 25).until(
        EC.visibility_of_element_located(teachers_elements.subjects_list_button)).click()
    target = browser.find_element_by_css_selector(
        '[data-qa-marker="submit-editable-class"]').location_once_scrolled_into_view
    try:
        marathon_button = wdw(browser, 25).until(
