...
def new_subjects_year(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_checkbox):
    ...
    mmyy = wdw(browser, 30).until(EC.element_to_be_clickable(card_data_page_elements.card_date)).send_keys(
        card_data.mmyy_1)
    cvv = wdw(browser, 30).until(EC.element_to_be_clickable(card_data_page_elements.cvv_num)).send_keys(
        card_data.cvv_1)
...
def old_subjects_year(browser, card_1, mmyy_1, cvv_1, paypass_1, choose_check_box):
    ...
    card_buy_click = wdw(browser, 30).until(
        EC.element_to_be_clickable(card_data_page_elements.card_buy_button)).click()
    browser.implicitly_wait(10)
    paypass = wdw(browser, 30).until(EC.element_to_be_clickable(cloud_page_elements.password_input)).send_keys(card_data.paypass_1)
    paypass_click = wdw(browser, 30).until(EC.element_to_be_clickable(cloud_page_elements.paypass_button)).click()
    paid_click = wdw(browser, 30).until(EC.visibility_of_element_located(congrat_page_elements.congrat_button))
